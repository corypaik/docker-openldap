  ...
  if [ "${KEEP_EXISTING_CONFIG,,}" == "true" ]; then
    ...
    if [ -e "$WAS_STARTED_WITH_TLS" ]; then
      ...
      if [ "${DISABLE_CHOWN,,}" == "false" ]; then
        ...
        chown openldap:openldap $PREVIOUS_LDAP_TLS_CRT_PATH $PREVIOUS_LDAP_TLS_KEY_PATH $PREVIOUS_LDAP_TLS_DH_PARAM_PATH
      fi
    fi
    ...
    if [ "${LDAP_TLS,,}" == "true" ]; then
      ...
      NEXT_WAIT_TIME=1
      MAX_WAIT_TIME=60
      while [ "$NEXT_WAIT_TIME" -le "$MAX_WAIT_TIME"] ; do
        ldapmodify -Y EXTERNAL -Q -H ldapi:/// -f ${CONTAINER_SERVICE_DIR}/slapd/assets/config/tls/tls-enable.ldif && break 2>&1 | log-helper debug
        NEXT_WAIT_TIME=$((NEXT_WAIT_TIME*2))
      done
      if [ "$NEXT_WAIT_TIME" -gt "$MAX_WAIT_TIME" ]; then
        exit 1
      fi
